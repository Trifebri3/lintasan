...
<div class="space-y-6 max-w-xl text-xs">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-extrabold text-gray-900">Edit Media Sosial</h1>
            <p class="text-xs text-gray-500 mt-1">Perbarui rincian atau sembunyikan tautan media sosial.</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.social-links.update', $socialLink->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="platform" name="platform" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-xs focus:border-brand-green focus:ring-1 focus:ring-brand-green outline-none transition bg-white font-medium text-gray-800">
                        @php
                            $currentPlatform = old('platform', $socialLink->platform);
                        @endphp
                        ...
                        <option value="other" {{ $currentPlatform == 'other' ? 'selected' : '' }}>Lainnya / Website</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" required value="{{ old('name', $socialLink->name) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 text-xs focus:border-brand-green focus:ring-1 focus:ring-brand-green outline-none transition font-semibold" placeholder="Contoh: @senyum_anaknegeri">
            ...
            <div>
                ...
                <input type="url" id="url" name="url" required value="{{ old('url', $socialLink->url) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 text-xs focus:border-brand-green focus:ring-1 focus:ring-brand-green outline-none transition font-mono" placeholder="Contoh: https://www.instagram.com/senyum_anaknegeri/">
            ...
            <div>
                ...
                <input type="text" id="description_id" name="description_id" value="{{ old('description_id', $socialLink->description_id) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 text-xs focus:border-brand-green focus:ring-1 focus:ring-brand-green outline-none transition" placeholder="Contoh: Ikuti foto kegiatan dan Reels lapangan kami.">
            ...
            <div>
                ...
                <input type="text" id="description_en" name="description_en" value="{{ old('description_en', $socialLink->description_en) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 text-xs focus:border-brand-green focus:ring-1 focus:ring-brand-green outline-none transition" placeholder="Contoh: Follow our field updates and photo logs.">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" required value="{{ old('sort_order', $socialLink->sort_order) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2 text-xs focus:border-brand-green focus:ring-1 focus:ring-brand-green outline-none transition font-bold" placeholder="1">
                ...
                <div>
                    ...
                    <label class="flex items-center gap-2 mt-2 cursor-pointer select-none">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $socialLink->is_active) ? 'checked' : '' }} class="w-4 h-4 text-brand-green border-gray-200 rounded focus:ring-brand-green/20">
            ...
            <div class="pt-4 border-t border-gray-50 flex justify-end gap-3">
                ...
                <button type="submit" class="bg-brand-green text-white text-xs font-bold px-6 py-2.5 rounded-lg hover:bg-brand-darkgreen shadow transition">Perbarui Media</button>
            </div>
        </form>
    </div>
</div>
